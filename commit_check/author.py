...
def check_allow_authors(checks: list, check_type: str, stdin_text: Optional[str] = None) -> int:
    if stdin_text is None and has_commits() is False:
        return PASS  # pragma: no cover
...
def check_ignore_authors(checks: list, check_type: str, stdin_text: Optional[str] = None) -> int:
    if stdin_text is None and has_commits() is False:
        return PASS  # pragma: no cover
...
def check_required_signoff_details(checks: list, commit_msg_file: str = "", stdin_text: Optional[str] = None) -> int:
    ...
    if ok:
        return PASS
    _print_failure(base, 'required signoff details', required_name or required_email or '')
    return FAIL

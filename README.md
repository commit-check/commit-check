...
### Require the Conventional Branch description grammar

`conventional_branch` checks the `<type>/` prefix; the description after the
`/` is accepted as written. `require_description_grammar` holds it to the
grammar [Conventional Branch](https://conventionalbranch.org/) specifies as
well: lowercase alphanumeric segments joined by single hyphens, with dots
allowed inside a segment (`feature/add-login-page`, `release/v1.2.0`). It is
off by default, because branch names generated by bots are not all
spec-conformant — Dependabot's grouped updates are named
`dependabot/npm_and_yarn/lodash-4.17.21`, which the grammar rejects.

```toml
[branch]
conventional_branch = true
require_description_grammar = true
# Dependabot's branches don't follow the grammar; exempt the bot by branch
# name -- this bypasses only the naming check (CC201):
allow_branch_names = ["dependabot/.+"]
# ...or by author -- this skips every branch check for that author,
# including require_rebase_target (CC202):
ignore_authors = ["dependabot[bot]"]
```

With the option on, `feature/Add_Login`, `feature/new--login` and
`release/v1.-2.0` are rejected, and the failure names the corrected branch
(`feature/add-login`) whenever the fix is unambiguous.
...
| `branch.require_description_grammar` | `false` | `--require-description-grammar` | `CCHK_REQUIRE_DESCRIPTION_GRAMMAR` | Also hold the description after `<type>/` to Conventional Branch's grammar — lowercase alphanumeric segments joined by single hyphens, dots allowed inside a segment (CC201). Off by default: bot-generated names such as Dependabot's `dependabot/npm_and_yarn/lodash-4.17.21` are not spec-conformant |

...
.. list-table::
   ...
   * - branch
     ...
     - Allowed branch types when conventional_branch is true. AI agent prefixes (``ai/``, ``claude/``, ``codex/``, ``copilot/``, ``cursor/``), bot prefixes (``dependabot/``), and automation prefixes (``renovate/``) are also included by default.
   * - branch
     - allow_branch_names
     - list[str]
     - [] (empty list)

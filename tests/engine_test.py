...
class TestCommitMessageValidatorEdgeCases:
    ...
    def test_validate_with_valid_conventional_commit(self):
        """Test validation with valid conventional commit message."""
        rule = ValidationRule(
            check="message",
...
class TestSubjectCapitalizationEdgeCases:
    ...
    def test_validate_merge_commit_passes(self):
        ...
        validator = SubjectCapitalizationValidator(rule)
        context = ValidationContext(stdin_text="merge: something")

        result = validator.validate(context)
        assert result == ValidationResult.PASS

    def test_validate_conventional_commit_capitalized(self):
        """Test conventional commit with capitalized description."""
        rule = ValidationRule(
            check="subject_capitalized",
            error="Subject must be capitalized",
        )
...
class TestSubjectImperativeEdgeCases:
    ...
    def test_validate_non_match_fallback(self):
        """Test that non-imperative verbs fail validation."""
        rule = ValidationRule(
            check="imperative",
            error="Use imperative mood",
        )
        validator = SubjectImperativeValidator(rule)
        context = ValidationContext(stdin_text="feat: Added something")
...
class TestSubjectLengthEdgeCases:
    ...
    def test_validate_unknown_check_type(self):
        """Test with unknown check type."""
        rule = ValidationRule(
            check="unknown_length_check",
            value=50,
        )
        validator = SubjectLengthValidator(rule)
        context = ValidationContext(stdin_text="any subject")
...
class TestAuthorValidatorEdgeCases:
    ...
    def test_validate_regex_first_then_ignored(self):
        """Test that regex is checked before ignored list."""
        rule = ValidationRule(
            check="author_name",
        ...
        validator = AuthorValidator(rule)
        context = ValidationContext(stdin_text="bot-user")
...
class TestMergeBaseEdgeCases:
    ...
    def test_find_target_branch_cleans_remote_prefix(self):
        """Test that _find_target_branch cleans remote prefixes."""
        rule = ValidationRule(check="merge_base", regex="^main$")
        validator = MergeBaseValidator(rule)
...
class TestCommitTypeEdgeCases:
    ...
    def test_validate_merge_commit_when_not_allowed(self):
        """Test that merge commits are rejected when not allowed."""
        rule = ValidationRule(
    ...
    def test_validate_unknown_check_type(self):
        """Test with unknown check type."""
        rule = ValidationRule(
            check="unknown_type_check",
            value=True,
        )
        validator = CommitTypeValidator(rule)
        context = ValidationContext(stdin_text="any message")
...
class TestCommitMessageValidatorComprehensive:
    @pytest.mark.benchmark
...
class TestSubjectCapitalizationComprehensive:
    @pytest.mark.benchmark
...
class TestSubjectImperativeComprehensive:
    @pytest.mark.benchmark
...
class TestSubjectLengthComprehensive:
    @pytest.mark.benchmark
